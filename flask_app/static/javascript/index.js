  ...
  .addEventListener('click', async () => {
    try {
      ...
      if (result.status === 'success') {
        ...
        document.getElementById('submitEvalsBtn').innerText =
          'Submit Evaluations';
...
function createItemCard(item, recs) {
  ...
  info.innerHTML = `
        <h2 class="item-title">${item.product_title || 'Untitled'}</h2>
        <p class="item-category"><strong>Main Category:</strong> ${
          item.main_category || 'N/A'
        }</p>
        <p class="item-rating"><strong>Rating:</strong> ${
